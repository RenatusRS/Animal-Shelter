$(function () {
    ...
    for (let i = 0; i < ads.length; i++) {
        if (ads[i]["user"] == logged) {
            $("#ads").append(`<div name="${i}" class="text-break pointer ad border rounded p-3 my-2"><h5>${ads[i]["name"]}</h5> ${ads[i]["text"]} <br> ${ads[i]["contact"]} </div>`)
        }
    }
    ...
    for (let i = 0; i < comments.length; i++) {
        if (comments[i]["user"] == logged) {
            ...
            $(`#${comments[i]["ad"]}`).append(`<p class="text-break">${comments[i]["text"]}</p>`)
        }
    }
